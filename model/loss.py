...
class HNetLoss(_Loss):
    ...
    def _hnet_loss(self):
        ...
        loss = torch.mean(torch.pow(self.gt_pts[0, :] - x_transformation_back.t()[0, :], 2))
    ...
    def _hnet(self):
        ...
        H_shape = 9
        H = torch.zeros((1, H_shape))
        H.scatter_(dim=1, index=H_indices, src=self.transformation_coefficient)
        H = H.view((3, 3))

        pts_projects = torch.matmul(H, self.gt_pts.t())

        Y = pts_projects[1, :].view(1, -1).t()
        X = pts_projects[0, :].view(1, -1).t()
        ...
        preds = torch.stack((x_preds, Y, Y_One), dim=1).squeeze().t()
        return (H, preds)
